...
test_that("Summary function adds RItools info", {
  ...
  match.df <- pairmatch(mdist(test.df), data = test.data.na)
  ...
  match.fmla <- pairmatch(mdist(test.fmla), data = test.data)
  ...
  match.env <- pairmatch(mdist(test.env), data = test.data)
