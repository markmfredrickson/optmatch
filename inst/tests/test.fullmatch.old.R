...
test_that("", {
  ...
  expect_warning(fullmatch(1 * (plantdist < 10)), "Without 'data' argument the order of the match is not guaranteed
  ...
  mhd2[mhd2 < 1] <- 1
  mhd2[mhd2 >= 1] <- 0
  f1 <- fullmatch(mhd2, data = nuclearplants)
  expect_true('optmatch' %in% class(f1))
})
