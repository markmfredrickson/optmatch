...
- Each of `@nodes$name`, `@arcs@matches@upstream`,
  `@arcs@matches@downstream`, `@arcs@bookkeeping$start` and
  `@arcs@bookkeeping$end` has precisely the same collection of levels.
  (The names of the units to be matched, and/or names for bookkeeping
  units.)  To be enforced as a part of the validity check.
- The levels of`@nodes$name`, etc may include references to units that
  don't appear in the corresponding nodes table.(B/c e.g.  they
...
- Distinct matchable units should not share levels.
- OTOH bookkeeping nodes for distinct subproblems will share the
  same level of `@nodes$name`.
...
- in `@nodes`, exactly 2 `as.character(names)` values per subgroup s.t. `is.na(upstream_not_down)`, appearing in the final 2 positions of `levels(@names$name)`: "`(_Sink_)`" and "`(_End_)`";
...
- `c()` for `MCFSolutions`: `rbind()`s the various constituent data frames, enforcing req that subproblems have distinct names
