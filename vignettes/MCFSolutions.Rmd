...
- `upstream_eq_rows`, logical (do upstream nodes of MCF representation correspond to rows of match distance matrix?);
...
- `kind`, character ("upstream", "downstream" or other, e.g. "bookkeeping");
- `supply`, integer; 
...
- `@matches`, data frame with columns
    ...
    + `upstreamnode`, character,
    + `downstreamnode`, character; 
...
- In terms of the network flow solution, presence of a row in `@matches` encodes a corresponding flow of 1; absence encodes flow 0.  (So having the row means its `upstreamnode`/`startnode` and `downstreamnode`/`endnode` nodes were matched, absence means not matched.)
...
- Each (`subproblem`, `upstreamnode`) or (`subproblem`, `startnode`) pair must appear as a (`subproblem`, `name`) pair in the NodeInfo table, with `kind=="upstream"`; each (`subproblem`, `downstreamnode`) or (`subproblem`, `endnode`) pair must appear as a (`subproblem`, `name`) pair in the NodeInfo table, with `kind=="downstream"`. 
