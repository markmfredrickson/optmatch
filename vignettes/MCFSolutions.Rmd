...
- `nodenames` is simultaneously the level set for `@nodes$name`, `@arcs@matches@upstream`,
  `@arcs@matches@downstream`, `@arcs@bookkeeping$start` and `@arcs@bookkeeping$end`.
...
```
- There can be `nodenames` units that don't appear in a nodes table, if e.g.
they represent subjects that were were excluded prior to matching, or their subproblem was found to be infeasible prior to being sent to the solver. 
...
- in `@nodes`, exactly 2 `as.character(names)` values per subgroup s.t. `is.na(upstream_not_down)`, strings beginning with "`(_Sink_)`" and "`(_End_)`";
