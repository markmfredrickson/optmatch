...
- Each (`subproblem`, `upstream`) pair must appear as a (`subproblem`, `name`) pair in the NodeInfo table, with `upstream_not_down==TRUE`; each (`subproblem`, `downstream`) pair must appear as a (`subproblem`, `name`) NodeInfo  pair, with `upstream_not_down==FALSE`.
- Similarly a (`subproblem`, `start`) or (`subproblem`, `end`) pair carried in this table must have in the NodeInfo table a corresponding (`subproblem`, `name`) entry.   
...
```r
object@subproblems %>%
with(flipped[match(object@nodes$subproblem,subproblem)]) %>%
