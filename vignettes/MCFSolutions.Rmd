...
- Each of `@arcs@matches@upstream`,
  `@arcs@matches@downstream`, `@arcs@bookkeeping$start` and
  `@arcs@bookkeeping$end` has precisely the same collection of levels.
  Namely `@nodes$name`, the names of the units to be matched plus names for 
  bookkeeping nodes.  To be enforced as a part of the validity check.
...
- Distinct matchable units should not share values of `@nodes$name`.
...
- in `@nodes`, exactly 2 `$names` values per subproblem s.t. `is.na(upstream_not_down)`: a string beginning with "`(_Sink_)`" and another beginning with "`(_End_)`";
