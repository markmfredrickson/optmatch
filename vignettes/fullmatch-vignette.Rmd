...
matching work-flow in *R*. Example *R* code will appear as italics
with a `>` indicating the command prompt. You may type this code
yourself --- each line is a command to *R*. Output will follow in a
...
were merged together in the above command.  We'll use the
```match_on()``` command, introduced in optmatch version 0.8, to
create distances.
...
for a match.  For instance, the following fails, generating an error:
...
In this default report, the measure of closeness is just whatever
distance you matched on.  For closeness in terms of some other
distance, use `matched.distances`.
...
The propensity scores are then stored as `psm$fitted.values`
...
To extract a matrix of distances on the propensity score (and store it
as "`psd`"), do
...
Finally, export your data as a .csv file (or a proprietary format if
...
*J. Econom.*), is bundled with several R packages, including "`{arm}`"
and "`Matching`."  To get it:
