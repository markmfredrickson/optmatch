...
- `hashed.distance`, character (hashed ID for a double precision distance); 
- `reso`, double  (multiplier applied to double-precision distance before rounding and handing off to solver); 
-  `exceedance`, double (estimate/upper bound for amount by which sum of matched distances may exceed true optimal value.); 
-  `CS_after_backtransformation`, logical.
...
- `CS_after_backtransformation` encodes whether the flow-price pair was found to be complementary slackness relationship even after backtransformation.  `FALSE` means either that the answer was no or that it hasn't been checked for this. 
...
- `subprobs`, a "SubProbInfo" object (see above); 
- `nodes`, a `NodeInfo` object (see above);
- `arcs`, a `ArcInfo` object (see above);
- `matchables`, a data frame with columns
    + `name`,
    + `subproblem`,
    + `kind`.
...
- `names<-` for `MCFSolutions` objects
