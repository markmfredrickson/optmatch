...
#' At most one of \code{mean.controls} and \code{omit.fraction} can be non-\code{NULL}.
#'
#' @param shared_treatment_excess While \code{max.controls} can limit the total
#' number of treated units in each set, \code{shared_treatment_excess} limits
#' the difference between the number of treated units sharing a control and
#' the number of sets that have more than one treated units. Lowering this
#' limit will tend to turn matched sets with many treated into more sets with
#' fewer treated in each.
#'
